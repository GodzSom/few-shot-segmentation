...
#
# There is a trick that is maybe not as well known (I should be grateful if anyone pointed out a reference):
#While it is very common to use targeted initialization such as the method of
#K. He (`torch.nn.kaiming_normal_` and `torch.nn.kaiming_normal_`), if we actually don't scale the parameter
#but multiply the factor separately, the same factor also scales the gradients and thus,
#for methods like stochastic gradient descent (SGD) the updates. (For optimizers that remove scaling such as Adam,
#    one would expect the effect to cancel with the modification in Adam's scaling.)
#
...
# The fist component is the mapping. It's a reasonably deep (8 layers)
#but otherwise very plain vanilla fully connected network aka Multi-Layer-Perceptron.
#The StyleGAN reference model uses Leaky ReLUs, so we do, too.
# Note that while we get an 18-channel (times 512 features) per image style matrix,
#all 18 channels will be the same.
#
# We also provide a truncation module pulling the upper layer's latent inputs towards the mean,
#but we don't activate it as the mean is not provided in the pre-trained network. We could run the mapping
#for a while and derive the truncation weights.
...
def get_style_gan(model=None):
    if model=='mapping_only':
...
def load_pretrained_stgan(path, model):
    style_gan = model
    state_dict = torch.load(path)
    model_dict = style_gan.state_dict()
    state_dict = {k: v for k, v in state_dict.items() if k in model_dict}

    print(len(state_dict.keys()))

    model_dict.update(state_dict)
    style_gan.load_state_dict(model_dict)

    print_network(style_gan)
    return style_gan

def gen_z(device, size=1):
    #nb_rows = 1
    #nb_cols = 1
    #nb_samples = nb_rows*nb_cols
    #depth = 512
    #rng = np.random.RandomState(23)
    #latent = torch.from_numpy( rng.standard_normal(size*depth).reshape(size, depth)).float()
    latent = torch.randn(1,size, 512, device=device)
    print(latent.shape)
    #if device is not None:
    #  latent = latent.to(device)
    #latents = torch.randn(nb_samples, 512, device=device)
    #print(latent.shape)
    #td = TensorDataset(latent)
    #print(td.shape)
    return latent

def z_sample(size, depth=512, seed=None, device=None):
    '''
    ...
    # trng = stats.truncnorm(0,1).rvs(size = size*depth)
    # trng = np.array(trng).reshape(size,depth)
    # #rng = np.random.RandomState(seed)
    # result = torch.from_numpy(trng).float()
    # if device is not None:
    #     result = result.to(device)

    rnd = np.random.RandomState(seed)
    z = rnd.randn(1, depth) # [minibatch, component]
    result = torch.from_numpy(z).float()
    # print(result)
    return result

def new_z_sample(size, depth=512, seed=None, device=None):
    '''
    Generate a standard set of random Z as a (size, z_dimension) tensor.
    With the same random seed, it always returns the same z (e.g.,
    the first one is always the same regardless of the size.)
    ...
    rnd = np.random.RandomState(seed)
    z = rnd.randn(6, depth) # [minibatch, component]
    result = torch.from_numpy(z).float()
    # print(result)
    return result
...
if __name__ == "__main__":
    '''
    checkpoint_path = '/home/ntt/gandissect/netdissect/checkpoint/karras2019stylegan-ffhq-1024x1024.for_g_all-b418e00b3f4f99b879c3e805b8c55e.pt'
    style_gan = get_style_gan()
    style_gan = load_pretrained_stgan(checkpoint_path, style_gan)
    model_dict = style_gan.state_dict()
    #print(style_gan)

    for key in model_dict.keys():
        print("{} : {}\n".format(key, model_dict[key].shape))
    exit()
    '''
    '''
    ...
    checkpoint_path = '/home/ntt/gandissect/netdissect/checkpoint/karras2019stylegan-ffhq-1024x1024.for_g_all-b418e00b3f4f99b879c3e805b8c55e.pt'
    #style_gan_m = get_style_gan(model='mapping_only')
    style_gan_s = get_style_gan(model='syn_only')
    #style_gan_m = load_pretrained_stgan(checkpoint_path, style_gan_m)
    style_gan_s = load_pretrained_stgan(checkpoint_path, style_gan_s)
    ...
    #style_gan_m.eval()
    #style_gan_m.to(device)
    style_gan_s.eval()
    style_gan_s.to(device)
    #latent = gen_z(device=device,size =18)
    #with torch.no_grad():
    #    latents = style_gan_m(latent)
    #latents = latents.cpu()
    #print(latent.shape)
    #exit()
    #np.save('test_wp_latent1.npy', latents)
    #latents = latents.to(device)

    a = np.load('/home/ntt/gandissect/netdissect/dlatent.npy')
    latent = torch.from_numpy(a).float()
    latent = latent.to(device)
